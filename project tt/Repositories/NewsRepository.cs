...
namespace ProjectFinal.Repositories
{
    ...
    public class NewsRepository : INewsRepository
    {
        ...
        {
            var query = db.News.Where(x => x.Status == 1).OrderByDescending(x => x.CreateDate).ToList();
        ...
        {
            var query = db.News.Where(x => x.Status==1 && x.NewCategoryId == id).OrderByDescending(x=>x.CreateDate).ToList();
            totalRow = query.Count();
            return query.Skip((page - 1) * pageSize).Take(pageSize);
        }
