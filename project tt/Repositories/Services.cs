...
namespace ProjectFinal.Repositories
{
    ...
    public class Services : IServices
    {
        ...
        {
            var query = db.Service.Where(x => x.Status == 1 && x.ToDate > DateTime.Now).OrderByDescending(x => x.CreateDate).ToList();
        ...
        {
            //var lst = db.Service.Where(y => y.ToDate >= DateTime.Now).OrderByDescending(y => y.CreateDate).ToList();
